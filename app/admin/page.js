...
export default function AdminPage() {
  ...
  const handleLogin = async (e) => {
    ...
    else { setIsAuthenticated(true); localStorage.setItem('lf_admin_auth', 'true'); fetchData(); }
  };

  const handleLogout = () => { setIsAuthenticated(false); localStorage.removeItem('lf_admin_auth'); };

  const fetchData = async () => {
    const { data: photos } = await supabase.from('photo_requests').select('*').order('created_at', { ascending: false });
    if (photos) setPhotoRequests(photos);

    const { data: edits } = await supabase.from('editing_requests').select('*').order('created_at', { ascending: false });
    if (edits) setEditRequests(edits);

    const { data: responses } = await supabase.from('editing_responses').select('*').order('created_at', { ascending: true });
    if (responses) {
      const grouped = {};
      responses.forEach(r => {
    ...
    const { data: portItems } = await supabase.from('portfolio_items').select('*').order('created_at', { ascending: false });
...
function EditingTicketCard({ request, responses, onSendResponse, defaultTemplate }) {
